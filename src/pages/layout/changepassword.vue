<template>
    <div class="bg-gray-100 h-screen p-10">
      <div class="container flex flex-col lg:flex-row mx-auto items-center justify-center p-10">
        <div class="left lg:w-1/2 mx-4 mb-6 lg:mb-0 text-center lg:text-left">
          <img class="w-40 lg:w-60 p-2 mx-auto lg:mx-0" src="/public/ServerAvatar.png" alt="ServerAvatar" />
          <div class="ml-5">
            <p class="text-lg lg:text-xl font-bold">Cloud Hosting Simplified.</p>
            <p class="text-md mt-1 mx-2 lg:mx-0">
              Effortlessly host PHP and Node.js applications on our PHP & Node.js Cloud Server Management Panel.
              Manage security, updates, and configurations with ease using our intuitive control panel
              <span class="font-bold">No Technical Expertise Required</span>.
            </p>
          </div>
        </div>
  
        <div class="right flex flex-col bg-white p-8 lg:p-10 mx-auto rounded-xl w-full max-w-md">
          <!-- <input
            v-model="email"
          ...
          <!-- <div class="flex text-center p-2 mt-6 items-center justify-center">
            <span class="text-sm cursor-pointer">
              <span class="font-bold hover:underline" @click="createPage">Create a Page</span>
  ...
  export default {
    ...
    methods: {
      async login() {
        ...
        try {
          const response = await axios.post('https://app.satesting1.homes/api/login', {
            email: this.email,
          ...
          console.log('Access token set in store:', authStore.accessToken); 
